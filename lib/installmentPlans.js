...
export const getInstallmentIndex = (plan, monthKey) => {
...
export const getInstallmentRomanNumeral = (index) => {
  if (index >= 1 && index <= ROMAN_NUMERALS.length) {
    return ROMAN_NUMERALS[index - 1];
  }
  return String(index);
};

export const formatInstallmentPaymentName = (planName, plan, monthKey) => {
  const index = getInstallmentIndex(plan, monthKey);
  if (!index) return planName;
  return `${planName} ${getInstallmentRomanNumeral(index)}`;
};
...
export const monthHasUserData = (monthData) => {
  ...
  return balance > 0
    ...
    || (monthData.incomeMin || 0) > 0
    || (monthData.incomeMax || 0) > 0
...
export const applyInstallmentPlansToMonth = (automaticPayments, plans, monthKey, hasUserData = true) => {
  ...
  const planPayments = (plans || [])
    ...
    .map((plan) => ({
      ...
      name: formatInstallmentPaymentName(plan.name, plan, monthKey),
...
export const getPlanProgressLabel = (plan, monthKey) => {
  ...
  const roman = getInstallmentRomanNumeral(index);
  return `${roman}/${getInstallmentRomanNumeral(plan.totalMonths) || plan.totalMonths}`;
};
